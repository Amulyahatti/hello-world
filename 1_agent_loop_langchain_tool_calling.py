...
#----- Tools (Langchina @tool decorator) -----#

@tool
def get_product_price(product: str) -> float:
    """Look up the price of a product in a catlog"""
...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    print(f"  >> Executing tool: apply_discount with input: price={price}, discount_tier={discount_tier}")
    discounts = {
        "bronze": 0.05,
        "silver": 0.10,
        "gold": 0.20,
...
def run_agent(question: str):
    ...
    # llm = init_chat_model(f"azure_openai:{MODEL}", temperature=0, api_version="2024-02-15-preview")
